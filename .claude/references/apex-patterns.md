...
public with sharing class AccountDomain extends fflib_SObjectDomain implements IAccountDomain {
    ...
    public override void onAfterInsert() {
        Logger.info('AccountDomain.onAfterInsert: ' + Records.size() + ' records').addTag('AccountDomain');
        Logger.saveLog();
    }
    ...
    private void checkStatusTransition(List<Account> accounts, Map<Id, Account> oldMap) {
        for (Account acc : accounts) {
...
public with sharing class AccountServiceImpl implements IAccountService {
    ...
    public void updateIndustry(Set<Id> accountIds, String newIndustry) {
        Logger.info('AccountServiceImpl.updateIndustry called for ' + accountIds.size() + ' accounts').addTag('AccountService');
        ...
        IAccountDomain domain = (IAccountDomain) Application.Domain.newInstance(accounts);
...
private class AccountServiceImplTest {
    ...
    static void updateIndustry_updatesAllRecords() {
        ...
        ((fflib_ISObjectUnitOfWork) mocks.verify(uowMock, mocks.times(3))).registerDirty(
...
public class AccountTestFactory {
    ...
    public static List<Account> createAccounts(Integer count) {
        ...
        for (Integer i = 0; i < count; i++) {
            accounts.add((Account) TestDataFactory.createSObject('Account', new Map<String, Object>{
                'Name'       => 'Test Account ' + i,
                'Industry'   => 'Technology',
                'Rating'     => 'Warm',
